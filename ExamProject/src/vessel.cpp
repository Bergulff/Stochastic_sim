...
// R4 - Simulation vessel implementation
...
// R4 - Reaction addition with validation
...
// R3 - Symbol table with error handling
...
// R4 - Reaction network construction
...
// R4 - Find next reaction to execute (Gillespie algorithm)
...
// R4 - Update all reaction delays based on current state
...
// R4 & R6 - Main simulation loop implementing Doob-Gillespie algorithm
void Vessel::simulate(double end_time, std::function<void(double, const std::vector<std::shared_ptr<Species>>&)> observer) {
    ...
    while (time < end_time) {
        ...
        if (dt == std::numeric_limits<double>::infinity() || !std::isfinite(dt)) {
            std::cout << "[SIM] No more reactions possible - terminating simulation\n";
            break;
        }
        ...
        if (next->can_proceed(species_)) {
            ...
            // Observe after reaction
            if (observer) observer(time, species_);
        } else {
            std::cout << "[SIM] Reaction cannot proceed - insufficient reactants\n";
            break;
        }
    }
}

// R3 - Species lookup using symbol table
...
// R2 - Human readable reaction description
